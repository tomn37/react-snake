...
export default class Board {
  ...
  getGameoverMessage(): undefined | string {
    const isGameOver =
      this.isOutOfBounds() ||
      this.hasHitSelf() ||
      (this.hasHitBadFood() && !this.crownActive);
    if (isGameOver) {
      setHiscore(this.getScore());
      if (this.hasHitBadFood() && !this.crownActive) {
        return "Don't eat the Baidehis!!";
      }
  ...
  private getFoodPositionRecurse(
    excludedPositions: (Position | undefined)[],
  ): Position {
